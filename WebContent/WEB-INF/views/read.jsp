...
<script type="text/javascript">
	...
	function reply_del_func(replyNum) {
		...
		if (result == true) {
			location.href = "replyDelete.do?replyNum=" + replyNum;
			return true;
		}
		return false
	}
		...
		$("#btnReply").click(function() {
			...
			$.ajax({
				...
				success : function(result) {
					location.href = "reply.do?boardNum=" + boardNum;
				},
				error : function() {
					alert('댓글 등록');
...
<%@ include file="header.jsp" %>
	<div id="readcontainer">
		...
		<p align="center">
			<a href="board.do"><button type="button" class="btn btn-info">목록</button></a>
			&nbsp;&nbsp;
		...
		<a onclick="del_func()" href="#" class="btn btn-info">삭제</a>
			...
			<c:if test="${sessionScope.loginNick != board.writer}">
				<a href="recommendForm.do?boardNum=${board.boardNum}" class="btn btn-info">추천</a>
			</c:if>
		</p>
		...
		<c:if test="${not empty replyPage.boardList}">
			...
			<center>
				<div id="paging">
					<font size="4">
					<c:forEach
							begin="${replyPage.startPage}" end="${replyPage.endPage}" var="p"
							step="1">
