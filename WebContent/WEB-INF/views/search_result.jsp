		...
		<div style="float: left; width: 33%; padding: 2px;">
			<table id="stat-table"
				...
				<thead>
					<tr>
						<c:if test="${i eq 0}" >
							<th colspan="3" style="background-color: #F29661;text-align: center;">Solo ${statInfo[i+serverMode].wins}승
								${statInfo[i+serverMode].losses}패
								승률${statInfo[i+serverMode].win_Ratio}%</th>
						</c:if>
						<c:if test="${i eq 1}">
							<th colspan="3" style="background-color: #3DB7CC;text-align: center;">Duo ${statInfo[i+serverMode].wins}승
								${statInfo[i+serverMode].losses}패
								승률${statInfo[i+serverMode].win_Ratio}%</th>
						</c:if>
						<c:if test="${i eq 2}">
							<th colspan="3" style="background-color: #FFA7A7;text-align: center;">Squad ${statInfo[i+serverMode].wins}승
								${statInfo[i+serverMode].losses}패
								승률${statInfo[i+serverMode].win_Ratio}%</th>
						</c:if>
					</tr>
